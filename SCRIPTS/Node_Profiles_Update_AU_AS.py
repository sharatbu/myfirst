...
sql = """
 select count(1) from WMChangedObject with (nolock)
"""
rows = crsr.execute(sql)
for row in rows:

    text= "Initial Provisioning Queue Count = "+ str(row[0]) + '\n'
    with open(local_file_path, "a+") as myfile2:
        myfile2.write(text)
##########################################################IP validation
...
################# Network profile with highest usage count --SQL
        ...
        sql = """
select top 1000 Name from BaseStation bs with (nolock) where dbid in
...
################################### Fetch node names from db script
        ...
        for row in rows:
            call_node_profile(nbif_username, nbif_password, row[0], SystemDefaultProfile, AdvancedConfigProfile, SecurityProfile,
        ...
        while iter < 4:
            ...
            now = datetime.now()
            for row in rows:
                text = "Provisioning Queue Count = " + str(row[0]) + ','+"Completed time :"+now.strftime("%Y-%m-%d %H:%M:%S") + '\n'
                with open(local_file_path, "a+") as myfile2:
                    myfile2.write(text)
                iter = iter + 1
                print(iter)
...
def call_node_profile(nbif_username, nbif_password,boxid,SystemDefaultProfile,AdvancedConfigProfile,SecurityProfile,CellAdvancedConfigurationProfile,RadioProfile):
################################# AirUnity
