...
with open(CONFIG_PATH) as myfile:
    ...
    bat_file_reboot= myvars['bat_file_reboot'].strip()
...
def nodes_reboot(SIM_machine,bat_file_name):
...
if res_stop_ip== True and res_reb_ib == True:
    ...
    print("inloop 2 ")
    nodes_reboot(com_off_port_no_start_ip,bat_file_reboot)
    end_count()
    logging.debug("20% node Online again making nodes Online")
    initial_count()
    nodes_reboot(server_ip_lydia_shut,bat_file_name)
    nodes_reboot(com_off_port_no_start_ip,bat_file_name)
    end_count()
